...
# -*- coding: utf-8 -*-
"""
Created on Wed Dec  2 18:35:36 2020
...
class Game:
    ...
    def switchOrder(self):#automatically chooses the best model
        self._total_order[0], self._total_order[1],self._total_order[2], self._total_order[3] = self._total_order[3], self._total_order[0],self._total_order[1], self._total_order[2]
    ...
    def war(self, change_position = True):
        ...
        for i in range(4):
            self._players_bet.append(self._players[ order.index(i)].bet(self._players_bet))#a trick here to tell that player 1 with order 0 players first
    ...
    def actualizeBet(self):#players compare their bet and their score
        self._players[0].actualize_bet()
        self._players[1].actualize_bet()
        self._players[2].actualize_bet()
        self._players[3].actualize_bet()
        
    def actualizePlay(self, memory, game_round):
        for i in range(4):
            new_state = memory
            new_state[i] = self._players[i]._hand
            self._players[i].actualize_play(new_state, game_round)
    ...
    def plotAverage(self):
        ...
        for i in range(self._n+1):
            average.append((dic_history[self._players[0]._name][i]+dic_history[self._players[1]._name][i]+dic_history[self._players[2]._name][i]+dic_history[self._players[3]._name][i])/4)
